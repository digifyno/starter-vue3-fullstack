...
describe('Passkey Routes', () => {
  ...
  describe('GET /api/auth/passkeys', () => {
    ...
    it('returns empty array when user has no registered passkeys', async () => {
    ...
    it('returns registered credentials with id, deviceName, and createdAt', async () => {
      const { query } = await import('../database.js');
      const createdAt = new Date('2024-01-15T10:00:00.000Z');
      vi.mocked(query).mockResolvedValueOnce({
        rows: [
          { id: 'pk-row-1', device_name: 'My Phone', created_at: createdAt },
          { id: 'pk-row-2', device_name: null, created_at: createdAt },
        ],
      } as any);
    ...
    it('does not expose credential_id or public_key in the response', async () => {
      const { query } = await import('../database.js');
      vi.mocked(query).mockResolvedValueOnce({
        rows: [
          {
            ...
            credential_id: 'raw-cred-id-bytes',
            public_key: Buffer.from([1, 2, 3]),
          },
        ],
      } as any);
      ...
      expect(res.statusCode).toBe(200);
      const body = JSON.parse(res.body);
      expect(body[0]).not.toHaveProperty('credential_id');
  ...
  describe('DELETE /api/auth/passkeys/:credentialId', () => {
    it('returns 401 without an auth token', async () => {
      const res = await app.inject({
        method: 'DELETE',
        url: '/api/auth/passkeys/some-id',
      });
      expect(res.statusCode).toBe(401);
    });
    ...
    it('deletes the credential and returns success', async () => {
      const { query } = await import('../database.js');
      vi.mocked(query).mockResolvedValueOnce({ rows: [{ id: 'pk-row-1' }] } as any);

      const res = await app.inject({
        method: 'DELETE',
        url: '/api/auth/passkeys/pk-row-1',
        headers: { authorization: 'Bearer valid-token' },
      });
      ...
      expect(vi.mocked(query)).toHaveBeenCalledWith(
        ...
        expect.arrayContaining(['pk-row-1', 'user-1']),
      );
    });
    ...
    it('returns 404 when credential does not exist', async () => {
      ...
      const res = await app.inject({
        method: 'DELETE',
        url: '/api/auth/passkeys/nonexistent-id',
        headers: { authorization: 'Bearer valid-token' },
      });
    ...
    it('returns 404 when credential belongs to a different user (IDOR protection)', async () => {
      const { query, requireAuth } = await import('../database.js').then(async (db) => {
        const auth = await import('../middleware/auth.js');
        return { query: db.query, requireAuth: auth.requireAuth };
      });

      // Authenticate as user-2
      vi.mocked(requireAuth).mockImplementationOnce(async (request: any) => {
        request.userId = 'user-2';
        request.userEmail = 'other@example.com';
      });

      // DB returns no rows because WHERE id = $1 AND user_id = $2 finds no match
      vi.mocked(query).mockResolvedValueOnce({ rows: [] } as any);

      const res = await app.inject({
        method: 'DELETE',
        url: '/api/auth/passkeys/pk-row-owned-by-user-1',
        headers: { authorization: 'Bearer valid-token' },
      });
    ...
    it('passes user_id in DELETE query to enforce ownership', async () => {
      const { query } = await import('../database.js');
      vi.mocked(query).mockResolvedValueOnce({ rows: [{ id: 'pk-row-1' }] } as any);
      ...
      // Verify user_id is in the query parameters (prevents IDOR)
      expect(vi.mocked(query)).toHaveBeenCalledWith(
